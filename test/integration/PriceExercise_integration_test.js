...
skip.if(developmentChains.includes(network.name)).
  describe('PriceExercise Integration Tests', async function () {
    ...
    beforeEach(async () => {
      ...
      priceExercise = await ethers.getContractAt('PriceExercise', APIConsumer.address)
    })

    it('Should successfully make an external API request and get a result', async () => {
      const transaction = await priceExercise.requestVolumeData()
      ...
      const result = await priceExercise.volume()
      console.log("API Latest Price: ", new web3.utils.BN(result._hex).toString())
